...
export const AttackPath: React.FC = () => {
    ...
    const [attackData, setAttackData] = useState<AttackComponent.AttackItems[]>([]);
    const [open, setOpen] = useState(false);
    const [placement, setPlacement] = useState<DrawerProps['placement']>('bottom');
    const [currentAttack, setCurrentAttack] = useState<AttackComponent.AttackItems | null>(null);
    const showDrawer = () => {
        setOpen(true);
      };
    ...
    useEffect(() => {
        ...
        }).then((res: AttackComponent.AttackResponse) => {
    ...
    useEffect(() => {
        if (chartRef.current && attackData.length > 0) {
            ...
            myChart.on('click', function (params) {
                const data = params.data as AttackComponent.AttackItems
